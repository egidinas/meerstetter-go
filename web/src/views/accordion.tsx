...
export function BottomAccordion({ title, sub, right, defaultOpen = true, storageKey, children }) {
...
function RoleSourceBadge({ channel }) {
  const confidence = MecomAPI.roleConfidence ? MecomAPI.roleConfidence(channel) : { kind: "warn", label: "unconfirmed", detail: "Channel role source is not available." };
  ...
  return (
    <div className="sig-strip">
      ...
      <div className="channels-row">
        ...
        {cells.length === 0 && <span className="dim mono small" style={{ padding: "4px 8px" }}>No channels apply for this signal.</span>}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="cmd-block">
      ...
      <div className="cmd-grid">
        {cells.map((c) => <ControlCell key={c.device_id + "/" + c.instance} signal={signal} channel={c} holderId={holderId} />)}
        {cells.length === 0 && <span className="dim mono small" style={{ padding: "4px 8px" }}>No channels apply.</span>}
      </div>
    </div>
  );
}
...
function ControlCell({ signal, channel, holderId }) {
  ...
  async function commit(overrideValue?) {
    ...
    try {
      ...
      const v = overrideValue !== undefined ? overrideValue : (isEnum ? parseInt(staged, 10) : stagedNum);
      ...
      toast.push({ kind: "ok", title: `${signal.name} → ${signal.unit ? v + " " + signal.unit : v}`, body: `${channel.device_id}/${channel.instance}` });
      setStaged(""); setConfirmText("");
    } catch (err) {
  ...
  return (
    <div className={["cmd-cell", changed || (isEnum && staged !== "") ? "staged" : "", signal.dangerous ? "danger" : "", unreach ? "unreach" : ""].join(" ")}
         title={MecomAPI.provenance(channel.device_id, signal.id, channel.instance)}>
      <div className="head">
      ...
      <div className="cur"><span>current</span><b>{curStr}{signal.unit && (signal.unit === "degC" ? " °C" : " " + signal.unit)}</b></div>
      {isTrigger ? (
        ...
        <>
          <div className="enum-row">
            {Object.entries(signal.enum || {}).map(([k, label]) => {
              ...
              return (
                <button key={k} className={isStaged ? "staged" : (isCur ? "cur" : "")} disabled={unreach}
                        onClick={() => isCur ? setStaged("") : setStaged(String(kNum))} title={label as string}>
                  {label as string}
                </button>
              );
            })}
          ...
          <div className="stage-row">
            ...
            <button className="send" disabled={!isValid || !confirmOK || unreach || busy || staged === ""} onClick={() => commit()}>
              {busy ? "…" : "Send"}
            </button>
          </div>
        </>
        ...
        <>
          <div className="stage-row">
            <input className="prefill" value={displayed} disabled={unreach} onChange={(e) => setStaged(e.target.value)} placeholder={curStr} />
            {changed && <button className="reset-btn" title="Discard change" onClick={() => setStaged("")}>↺</button>}
          </div>
          <div className="stage-row">
            <span className="range-hint">
              {signal.min !== undefined || signal.max !== undefined ? `${signal.min ?? "—"} … ${signal.max ?? "—"}` : "no range"}
            </span>
            <button className="send" disabled={!isValid || !changed || !confirmOK || unreach || busy} onClick={() => commit()}>
              {busy ? "…" : "Send"}
            </button>
          </div>
          {needsConfirm && (
...
export function SignalsAccordion({ role, channels, title, sub, defaultOpen, storageKey, scopeLabel }) {
  useGatewayTick();
  ...
  return (
    <BottomAccordion title={title} sub={sub || summary} defaultOpen={defaultOpen} storageKey={storageKey}
      right={
        <>
