/* ============================================================
   Meerstetter Gateway — Signal Dictionary view
   Hierarchy: Group → Subgroup → Signal → (Device, Instance)
   Two tabs: Telemetry (monitor) / Telecommands (control)
   Each signal block has a per-channel live value table and
   a graph-assignment panel.
   ============================================================ */
/* global React, MecomAPI, Pill, Chip, Panel, useToast, useLiveValue,
   useGatewayTick, categorizeError, useAssignments, WALLS, wallForDevice,
   signalAddress, channelColor */

const { useState, useEffect, useRef, useMemo } = React;

function SignalDictionaryView() {
  useGatewayTick();
  const [tab, setTab] = useState("monitor");
  const [selected, setSelected] = useState(null); // {group, subgroup}
  ...
  // Filter by role (monitor vs control)
  const filteredByRole = useMemo(() => {
    return catalogue.filter((p) => p.role === tab);
  }, [tab, catalogue]);
  ...
  // Default selection when tab changes
  useEffect(() => {
  ...
  // Signals matching the current selection (the right-hand main area)
  ...
  return (
    <div className="dict">
      ...
      <div className="dict-main">
        ...
        {selected && selected.group === "Metadata" && (
          <ChannelsEditor />
        )}
      </div>
    </div>
...
function GroupSection({ group, sgrps, selected, setSelected, tab, query }) {
  ...
  if (query) {
    // When searching, only show subgroups with matches
    const filtered = {};
  ...
  return (
    <div className="dict-grp">
      ...
      {open && Object.entries(sgrps).map(([sgrp, items]) => {
        ...
        // Role mix in this subgroup (just for the dot)
...
function SignalBlock({ signal, channels, holderId, tab }) {
  ...
  // Walls relevant for this signal — always the two fleet heroes plus per-device
  const wallOptions = useMemo(() => {
    ...
    if (signal.role === "monitor") {
      ...
      // For telecommands, "where the command card lives"
      if (signal.applicableModes && signal.applicableModes.includes("temp")) {
    ...
    <div className="dict-signal-block">
      ...
      <div className="dict-signal-body">
        ...
        <div className="dict-assign">
          ...
          {wallOptions.length === 0 && <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--muted)" }}>
            No applicable walls.
          </div>}
          {wallOptions.map((opt) => (
            <SignalAssignmentRow key={opt.wall_id}
                                 signal={signal}
                                 wall={opt}
...
function SignalAssignmentRow({ signal, wall, channels, assigns }) {
  // "Show in" for monitor signals is per-channel; for fleet heroes we add all
  // applicable channels at once with one toggle (default behaviour).
...
function CommandRows({ signal, channels, holderId }) {
  return (
    <table className="dict-table">
      ...
      <tbody>
        {channels.map((c) => <CommandRow key={c.device_id + "/" + c.instance}
...
/* ============================================================
   Channels editor — under Metadata group
   ============================================================ */
function ChannelsEditor() {
  ...
  // Show 2 channels (inst 1, inst 2) per device, with mode toggle
  const rows = [];
  devices.forEach((d) => {
    [1, 2].forEach((inst) => {
...
Object.assign(window, { SignalDictionaryView, ChannelsEditor });
