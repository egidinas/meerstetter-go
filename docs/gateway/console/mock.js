...
(function () {
  ...
  /* ---------- Catalogue (matches UI_GRAPH_WALL_CONTRACT) ---------- */
  const CATALOGUE = [
    /* ── Thermal · monitor ────────────────────────────────────── */
  ...
  // hasCascade: appears when data exists (per contract). Only tec-75/1 has it.
  // role_source: per contract — when gateway exposes operating-mode/config-derived
  // role this becomes "live" or "configured"; today the UI assumes locally.
  ...
  const SCENARIOS = {
    ...
    mixed: {
      ...
      "tec-81/2": { setV: 3.3, setI: 0.18, bound: true, opMode: 3 },  // hit current limit → CC
    ...
    mock.channels.forEach((ch) => {
      ...
      if (ch.role === "temp") {
        mock.channelState[k] = {
          ...
          outputEnable: 1,
          opMode: s.opMode ?? 1, // 1 = TEC closed-loop
          bound: s.bound !== false,
        ...
        mock.channelState[k] = {
          ...
          opMode: s.opMode ?? 2, // 2 = PSU CV; 3 = PSU CC
          outputEnable: 1,
          sinkT: 22.0 + (Math.random() - 0.5) * 0.6,
          bound: s.bound !== false,
          rejectWrites: !!s.rejectWrites,
  ...
  /* ---------- Activity feed (contract-rich rows) ---------- */
  ...
  const API = {
    ...
    readValue(deviceId, paramId, instance) {
      ...
      let v;
      if (ch.role === "temp") {
        // Cascade reads as missing when channel.hasCascade is false
        if (paramId === 52200) {
    ...
    // Setpoints (for supply settings table display: show set + actual side by side)
    ...
    write(deviceId, req, leaseToken) {
      ...
      const prev = (() => {
        if (!ch) return null;
        try { return API.readValue(deviceId, param, inst).value; } catch (_) { return null; }
      })();
      if (!lease || lease.token !== leaseToken) {
        const err = new Error("lease invalid or expired");
        err.status = 423;
      ...
      if (!ch || !ch.bound) {
        const err = new Error("transport unreachable");
        err.status = 503;
      ...
      if (ch.rejectWrites) {
        const err = new Error("device rejected: out of valid range");
        err.status = 409;
      ...
      if (ch.role === "temp") {
        ...
        if (param === 1021) ch.setV = value;       // contract: 1021 = output_voltage_v
        if (param === 1020) ch.setI = value;       // contract: 1020 = output_current_a
        if (param === 2010) ch.outputEnable = value;
        if (param === 2040) ch.opMode = value;
      }
    ...
    acquireLease(deviceId, holder, ttl) {
      ...
      if (existing && existing.holder !== holder) {
        const err = new Error("device already leased by " + existing.holder);
        err.status = 423;
        return Promise.reject(err);
      }
      const lease = {
  ...
  // Format helpers
  API.formatValue = function (v, unit, paramId) {
  ...
  // Series role helpers (signalforge palette)
  API.roleForParam = function (paramId) {
    if (paramId === 3000) return "cmd";                           // target (yellow)
    if (paramId === 1000) return "actual";                         // object (cyan)
    if (paramId === 1001) return "ghost";                          // sink (grey)
    if (paramId === 52200) return "aux";                           // cascade (lilac)
    if (paramId === 1021) return "actual";                         // voltage (cyan)
    if (paramId === 1020) return "dut";                            // current (pink)
    if (paramId === 1022) return "aux";                            // power (lilac)
    return "ghost";
  };
  API.colorForRole = function (role) {
    return {
      cmd:    "var(--series-cmd)",
  ...
  API.provenance = function (deviceId, paramId, instance) {
    const dev = DEVICES_BASE.find((d) => d.id === deviceId);
