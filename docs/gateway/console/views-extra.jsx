...
function SequencerView({ deviceId }) {
  useGatewayTick();
  ...
  const [stepStatuses, setStepStatuses] = useState({}); // {stepId: status}
  ...
  // Compute per-step x positions (as fraction of total duration)
  ...
  function pushLog(stepId, status, msg) {
    setStepLog((cur) => [...cur, { stepId, status, msg, t: new Date() }]);
  }
  ...
  function startRun() {
    ...
    const tick = () => {
      const elapsed = (performance.now() - startRef.current) / 1000; // seconds
      ...
      setStepStatuses(sst);
      // Emit log lines as steps complete
      setStepLog((cur) => {
        const out = cur.slice();
        layout.forEach((s) => {
  ...
  function togglePause() {
    setPaused((cur) => {
      if (cur) {
        // resume: shift start so elapsed continues
        startRef.current = performance.now() - progress * totalDuration * 1000;
        if (timerRef.current) cancelAnimationFrame(timerRef.current);
        const tick = () => {
          // re-trigger run loop
          startRun.tickShim && startRun.tickShim();
        };
        // simpler: just call startRun without reset
        setRunning(true);
        // schedule a frame
        timerRef.current = requestAnimationFrame(() => {
          // continue the same loop manually:
          const loop = () => {
  ...
  // Lane width math (canvas SVG inside .gantt is implicit; we use absolute positioning on tracks)
  return (
    <div className="seq">
      <div>
        ...
        <Panel title="Run controls" right={<Chip kind="warn">planned backend</Chip>} flush>
          <div style={{ padding: 12, display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <div className="run-controls">
              ...
              {running && !paused && (
                <button className="btn" onClick={togglePause}>Pause</button>
              )}
              {running && paused && (
...
function Gantt({ script, layout, progress, stepStatuses }) {
  const total = layout[layout.length - 1]?.end || 1;
  return (
    <div className="gantt">
      <div className="ruler">
        ...
        <div key={t} className="lane">
          ...
          <div className="track">
            {layout.map((s, i) => {
              ...
              return (
                <div key={s.id}
                     className={["step", s.kind === "send_command" ? "send" : s.kind === "wait" ? "wait" : s.kind === "wait_stable" ? "stable" : s.kind === "assert" ? "assert" : "wait", status === "ok" ? "ok" : "", status === "run" ? "run" : "", status === "fail" ? "fail" : ""].join(" ")}
...
/* ============================================================
   PID ADVISOR
   ============================================================ */
function PIDAdvisor({ deviceId, onDeviceChange }) {
  useGatewayTick();
  ...
  const [response, setResponse] = useState(null); // { ts, obj, target }
      ...
      <div className="right">
        <Panel title="Current gains" meta="MeCom #2020 / #2021 / #2022">
          <div className="gain-row">
            <div className="gain">
              <div className="lbl">Kp</div>
              <div className="now">{kp.value != null ? kp.value.toFixed(2) : "—"}</div>
            </div>
            <div className="gain">
              <div className="lbl">Ti</div>
              <div className="now">{ti.value != null ? ti.value.toFixed(2) : "—"} <span className="dim">s</span></div>
            </div>
            <div className="gain">
              <div className="lbl">Td</div>
              <div className="now">{td.value != null ? td.value.toFixed(2) : "—"} <span className="dim">s</span></div>
            </div>
          </div>
        </Panel>

        <Panel title="Advisor" right={<>
          <Chip kind="warn">apply planned</Chip>
          {suggested && <Chip kind="accent">Ziegler-Nichols heuristic</Chip>}
        </>}>
          {!suggested && (
            <div className="dim mono" style={{ fontSize: 12, lineHeight: 1.6 }}>
              Capture a step response to see recommendations. The advisor estimates process gain & dead-time from rise/settling and proposes gain deltas. Backend gain writeback remains planned.
            </div>
          )}
          {suggested && (
            <>
              <div className="gain-row">
                <div className="gain">
                  <div className="lbl">Kp</div>
                  <div>
                    <span className="now">{kp.value?.toFixed(2)}</span>
                    <span className="arrow">→</span>
                    <span className="new">{suggested.kp.toFixed(2)}</span>
                  </div>
                </div>
                <div className="gain">
                  <div className="lbl">Ti</div>
                  <div>
                    <span className="now">{ti.value?.toFixed(2)}</span>
                    <span className="arrow">→</span>
                    <span className="new">{suggested.ti.toFixed(2)}</span>
                  </div>
                </div>
                <div className="gain">
                  <div className="lbl">Td</div>
                  <div>
                    <span className="now">{td.value?.toFixed(2)}</span>
                    <span className="arrow">→</span>
                    <span className="new">{suggested.td.toFixed(2)}</span>
                  </div>
                </div>
              </div>
        ...
        <Panel title="Lease" flush>
          <div style={{ padding: 12 }}>
            {lease ? (
              <Pill kind={youHold ? "info" : "warn"}>{youHold ? "you hold" : "held by " + lease.holder}</Pill>
            ) : <Pill kind="ok">free</Pill>}
          </div>
        </Panel>
      </div>
    </div>
...
function analyzeStep(response) {
  ...
  // Steady-state err: average of last 10 samples
...
function suggestGains(current, a) {
  ...
  if (a.overshoot > 15) {
    dKp *= 0.75;
    rationale.push(`Overshoot ${a.overshoot.toFixed(1)}% is high → reduce Kp by 25%.`);
  } else if (a.riseTime > 8) {
    dKp *= 1.2;
    rationale.push(`Rise time ${a.riseTime.toFixed(1)}s is slow → raise Kp by 20%.`);
  }
  if (a.settling > 12) {
    dTd *= 1.25;
    rationale.push(`Settling ${a.settling.toFixed(1)}s long → add 25% derivative action.`);
  }
  if (Math.abs(a.sse) > 0.05) {
    dTi *= 0.85;
    rationale.push(`Steady-state error ${a.sse.toFixed(3)}°C → shorten Ti by 15%.`);
  }
  if (rationale.length === 0) {
    rationale.push("Response within tolerance — no changes recommended.");
  }
  return {
    kp: +(kp * dKp).toFixed(3),
    ti: +(ti * dTi).toFixed(3),
  ...
  streams: [
    {
      ...
      primary_key: ["seq"],
      fields: ["seq", "time", "target_id", "device_id", "parameter", "value", "unit", "quality", "source_path"],
    },
    {
      name: "can_frames",
      ...
      primary_key: ["time", "interface", "id", "dlc", "data_hex"],
      fields: ["seq", "time", "interface", "id", "dlc", "data_hex", "source"],
    },
    {
      name: "command_events",
      ...
      primary_key: ["seq"],
      fields: ["seq", "time", "target_id", "write_path", "status", "owner", "message"],
    },
    {
    ...
    },
    {
      ...
      primary_key: ["wall_id", "tile_id"],
      fields: ["generated_at", "wall_id", "tile_id", "target_id", "kind", "section", "position"],
    },
  ],
};
...
function ArchiveView() {
  ...
  function toggleStream(name) {
    setStreamSel((cur) => ({ ...cur, [name]: !cur[name] }));
  }
  function exportNow() {
    const picked = Object.entries(streamSel).filter(([, v]) => v).map(([k]) => k);
    if (!picked.length) {
      toast.push({ kind: "warn", title: "Pick at least one stream" });
      return;
    }
    if (format === "hdf5") {
      toast.push({ kind: "warn", title: "HDF5 is planned", body: "Stream contract preserved; writer not implemented." });
      return;
    }
    toast.push({ kind: "ok", title: "Export queued",
                 body: `${picked.length} stream(s) · ${format} · range ${range}` });
  }

  return (
    <div className="arc">
      <Panel title="Export archive" right={
        <>
          <span className="dim mono" style={{ fontSize: 11 }}>route GET /api/log/export</span>
        </>
      }>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr auto", gap: 10, alignItems: "end" }}>
          ...
          <div>
            ...
            <div className="fmt-row">
              {formats.map((f) => (
                <button key={f.id}
                        className={"fmt " + (format === f.id ? "on" : "") + (f.planned ? " planned" : "")}
...
function SettingsView() {
  ...
  function set(patch) {
    const next = MecomAPI.saveSettings(patch);
    setS(next);
  }
  return (
    <div className="set">
      <Panel title="Gateway connection">
        <div style={{ display: "grid", gap: 10 }}>
          ...
          <div>
            ...
            <div className="dim small mono" style={{ marginTop: 4 }}>
              Used as the <code>holder</code> in <code>POST /api/devices/&#123;id&#125;/lease</code>. No auth yet — assume browser session ID.
            </div>
          </div>
        </div>
      </Panel>
      ...
      <Panel title="Workspace">
        <div style={{ display: "grid", gap: 10 }}>
          ...
          <div>
            ...
            <button className="btn sm" onClick={() => { Object.keys(localStorage).filter((k) => k.startsWith("mecomgw.pins.") || k.startsWith("mecomgw.cards.")).forEach((k) => localStorage.removeItem(k)); location.reload(); }}>
              Reset pinned params & input cards
            </button>
          </div>
        </div>
      </Panel>
...
Object.assign(window, { SequencerView, PIDAdvisor, ArchiveView, SettingsView });
